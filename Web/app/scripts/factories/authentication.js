...
app.factory('auth', ['$http', '$window', function ($http, $window) {
    ...
    auth.register = function (user) {
        return $http.post('localhost:8080/backend/api/users/register', user).success(function (data) {
    ...
    auth.login = function (user) {
        return $http.post('localhost:8080/backend/api/users/login', user).success(function (data) {
            auth.saveToken(data.token);
            auth.setHeader();
        });
